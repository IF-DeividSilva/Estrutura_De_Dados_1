 ...
 int main(char const *argv[])
 {
    ...
    escolha = verificaVisita();
        ...
        if (escolha == 1)
        {
         ...
         printf("\nConcretizada\n");
         imprimeFilaDinamica(&concretizadas);
         }else{
        ...
        printf("\nNão concretizada\n");
        imprimeFilaDinamica(&naoConcretizadas);
        }
    }
    while(concretizadas.quantidadeElementos != Tam)
    {
       recebeArq(argv, &iniciais, n); 
       for ( i = 0; i < Tam; i++)
    {
    printf("\n A visita na casa [%s] \n foi concretizada?", iniciais.elemento[i].chave);
//retorna a escolha do usuario por meio de funcao de verificacao
    escolha = verificaVisita();
    if (escolha == 1)
    {
// caso tenha sido concretizada chama a funcao enfileiraFilaDinamica e enfileira na fila de concretizadas
        enfileiraFilaDinamica(&concretizadas, &iniciais, n); 
    }else{
 // caso nao tenha sido concretizada chama a funcao enfileiraFilaDinamica e enfileira na fila de nao concretizadas 
        enfileiraFilaDinamica(&naoConcretizadas, &iniciais, n);    
    }  
    }
    }
    imprimeFila(&iniciais);
    return 0;
}

  
void recebeArq(char const *argv[],FilaEstatica* fe, int n){
    static FILE *arq1;
    char c;
    char linha[200];
    int contador = 0;
